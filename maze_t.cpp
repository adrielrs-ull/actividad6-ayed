...
bool
maze_t::solve(dll_t<pair_t<int>>& camino_salida)
{
  return solve_(i_start_, j_start_, camino_salida);
}
...
{ 
  ...
  if ((i >= 1 && i <= matrix_.get_m()) && (j >= 1 && j <= matrix_.get_n()) && (matrix_.at(i, j) != WALL_ID) && (visited_.at(i, j) != true)) {
    return true;
  }
  return false;
}
...
bool
maze_t::solve_(const int i, const int j, dll_t<pair_t<int>>& camino_salida)
{
  // CASO BASE:
  ...
  for (int direccion = N; direccion <= W; direccion++) {
    int eje_x = i + i_d[direccion];
    int eje_y = j + j_d[direccion];
    if (is_ok_(eje_x, eje_y) && solve_(eje_x, eje_y, camino_salida)) {
      if (i != i_start_) {
        matrix_(i, j) = PATH_ID;
        //FASE III
        //Guardar la solución en alguna estructura de datos tipo pair doble enlazaza (i, j)
        pair_t<int> paso(i, j);
        camino_salida.push_back(new dll_node_t<pair_t<int>>(paso));
      }
      return true;
    }
  }
  ...
  visited_(i, j) = false;
  return false;
  //quitamos del camino lo introducido
  camino_salida.pop_back();
}
